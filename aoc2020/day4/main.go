...
func main() {
	fmt.Println("Part 1:", ValidatePassportPartOne(input))
	fmt.Println("Part 2:", ValidatePassportPartTwo(input))
}

func ValidatePassportPartOne(input []string) int {
	var count int
	for _, p := range input {
		isValid := true
		for k, v := range rules {
...
func ValidatePassportPartTwo(input []string) int {
	byr := regexp.MustCompile("byr:([0-9]{4})")
	iyr := regexp.MustCompile("iyr:([0-9]{4})")
	eyr := regexp.MustCompile("eyr:([0-9]{4})")
	hgt := regexp.MustCompile("hgt:([0-9]{2,3})(cm|in)")
	...
	for _, p := range input {
		...
		v = ecl.FindStringSubmatch(p)
		if len(v) != 2 {
			continue
		}
		pp.ecl = v[1]

		v = pid.FindStringSubmatch(p)
		if len(v) != 2 {
			continue
		}
		if len(v[1]) > 9 {
...
var rules = map[string]*regexp.Regexp{
	...
	// "cid": regexp.MustCompile("cid:.*"), <- doesn't need this
}
