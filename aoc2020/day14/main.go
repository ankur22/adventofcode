...
func SumBitmask(input []string) int {
	...
	for _, in := range ii {
		for _, me := range in.mem {
			if _, ok := addrs[me.addr]; !ok {
				addrs[me.addr] = make([]byte, 36)
			}
			for j := range in.mask {
				if in.mask[len(in.mask)-1-j] != x {
					addrs[me.addr][35-j] = in.mask[35-j]
				} else if j < len(me.value) {
					addrs[me.addr][35-j] = me.value[len(me.value)-1-j]
				} else {
					addrs[me.addr][35-j] = '0'
				}
			}
		}
	}

	max, _ := strconv.ParseInt("111111111111111111111111111111111111", 2, 64)

	var num int
	for _, v := range addrs {
		// Remove the empty byte elements
		v = bytes.Trim(v, "\x00")
		str := string(v)
		...
		if err != nil {
			panic("can't parse binary to decimal int")
		}
		// fmt.Println(n)
		num += int(n)
	}
	...
	if int64(num) > max {
		fmt.Println("max reached at end")
	}
...
func parse(input []string) []instruction {
	...
	for _, line := range input {
		if strings.Contains(line, "mask") {
			...
			inst = &instruction{}
			inst.mask = []byte(strings.Replace(line, "mask = ", "", 1))
			continue
		}
		p := r.FindStringSubmatch(line)
		addr, err := strconv.ParseInt(p[1], 10, 64)
		if err != nil {
			panic("can't parse int (addr)")
		}
		value, err := strconv.ParseInt(p[2], 10, 64)
		if err != nil {
			panic("can't parse int (value)")
		}
		m := mem{
			addr:  int(addr),
			value: []byte(strconv.FormatInt(value, 2)),
		}
		inst.mem = append(inst.mem, m)
	}
	if inst != nil {
		rtn = append(rtn, *inst)
	}
