...
func main() {
	fmt.Println("Part 1:", NumValidPartOne(parse(input)))
	fmt.Println("Part 2:", NumValidPartTwo(parse(input)))
}
...
func NumValidPartOne(rows []Row) int {
	var valid int
	for _, r := range rows {
		c := int64(strings.Count(r.Password, r.Rule))
		if c >= r.StartRange && c <= r.EndRange {
			valid++
		}
	}
	return valid
...
func NumValidPartTwo(rows []Row) int {
	var valid int
	for _, r := range rows {
		if int64(len(r.Password)) < r.StartRange {
			continue
		}

		if int64(len(r.Password)) < r.EndRange {
			continue
		}

		if (string(r.Password[r.StartRange-1]) == r.Rule && string(r.Password[r.EndRange-1]) == r.Rule) ||
			(string(r.Password[r.StartRange-1]) != r.Rule && string(r.Password[r.EndRange-1]) != r.Rule) {
			continue
		}
...
func parse(input []string) []Row {
	...
	rr := make([]Row, 0, len(input))
	for _, i := range input {
		ss := r.FindStringSubmatch(i)

		s, err := strconv.ParseInt(ss[1], 10, 64)
		if err != nil {
			panic("can't parse start")
		}

		e, err := strconv.ParseInt(ss[2], 10, 64)
		if err != nil {
			panic("can't parse start")
		}
