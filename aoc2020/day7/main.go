...
func main() {
	roots := ParseRules(input)
	fmt.Println("Part 1:", FindNumRuleCombination("shiny gold", roots))
	fmt.Println("Part 2:", FindNumBags("shiny gold", roots))
}
...
func FindNumBags(bag string, rules map[string]*Rule) int {
	v, ok := rules[bag]
	if !ok {
		panic("can't find the bag in the rule") // lazy
	}
	return countBags(v)
}

func countBags(rule *Rule) int {
	var rtn int
	for k, v := range rule.Children {
		rtn += rule.Counts[k]
		if len(v.Children) > 0 {
			rtn += countBags(v) * rule.Counts[k]
		}
	}
	return rtn
}

func FindNumRuleCombination(bag string, rules map[string]*Rule) int {
	var count int
	for _, rule := range rules {
		if find(rule.Children, bag) != nil {
			count++
		}
	}
	return count
...
func ParseRules(input []string) map[string]*Rule {
	r1 := regexp.MustCompile("^([a-z ]+) (bag|bags) contain ")
	r2 := regexp.MustCompile("(([0-9]+) ([a-z ]+) (bag|bags))+")

	root := make(map[string]*Rule)
	for _, i := range input {
		ru := r1.FindStringSubmatch(i)
		if ru == nil || len(ru) == 1 {
			panic("cannot find rule") // lazy!!
		}
		...
		if rule == nil {
			...
			root[rule.BagColour] = rule
			// fmt.Println("P: ", rule.BagColour)
		}
		...
		rus := r2.FindAllStringSubmatch(i, -1)
		if rus == nil {
			panic("cannot find bags contained in main bag") // lazy!!
		}

		for _, ru := range rus {
			for i := 1; i < len(ru); i += 4 {
				c, err := strconv.ParseInt(ru[i+1], 10, 64)
				if err != nil {
					panic("can't parse string to int") // lazy!!
				}

				ruleC := find(root, ru[i+2])
				if ruleC == nil {
					ruleC = &Rule{
						BagColour: ru[i+2],
						Counts:    make(map[string]int),
					...
					root[ruleC.BagColour] = ruleC
					// fmt.Println("C: ", ruleC.BagColour)
				}

				rule.Counts[ru[i+2]] = int(c)
				rule.Children[ruleC.BagColour] = ruleC
			}
		}
	}
...
func find(rules map[string]*Rule, bag string) *Rule {
	var rtn *Rule
	for _, r := range rules {
		if r.BagColour == bag {
			return r
		}
		rtn = find(r.Children, bag)
		if rtn != nil {
			return rtn
		}
	}
